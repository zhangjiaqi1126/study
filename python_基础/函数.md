...
`import`的用法在后续[模块](https://liaoxuefeng.com/books/python/module/index.html)一节中会详细介绍。
...
1. **递归终止条件**：每个递归函数都需要一个终止条件，以防止无限递归。在上面的例子中，终止条件是`n == 0`。
2. **避免栈溢出**：如果递归深度太大，可能会导致栈溢出错误。可以通过优化算法、使用尾递归等方法来减少递归深度。
3. **效率问题**：递归函数可能比循环更慢，因为每次函数调用都会消耗额外的内存和处理时间。